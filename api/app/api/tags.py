...
def list_tags(
    ...
    return db.query(Tag).order_by(Tag.name.asc()).limit(500).all()
...
@router.post("/customers/{customer_id}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response)
def add_tag(
    customer_id: UUID,
    payload: TagCreate,
    db: Session = Depends(get_db),
...
@router.delete("/customers/{customer_id}/{tag_name}", status_code=status.HTTP_204_NO_CONTENT, response_class=Response)
def delete_tag(
    customer_id: UUID,
    tag_name: str,
    db: Session = Depends(get_db),
