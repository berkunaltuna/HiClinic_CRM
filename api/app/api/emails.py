...
def _select_template(db: Session, *, template_id: UUID | None, template_name: str | None, customer: Customer) -> Template:
    ...
    tpl = db.query(Template).filter(Template.channel == "email", Template.name == template_name, Template.language == preferred_lang).first()
    if tpl:
        return tpl

    tpl = db.query(Template).filter(Template.channel == "email", Template.name == template_name, Template.language == "und").first()
    if tpl:
        return tpl
...
def send_email(
    ...
    provider_message_id = provider.send_email(to_email=customer.email, subject=rendered_subject, body=rendered_body)
